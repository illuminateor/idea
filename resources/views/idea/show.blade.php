<x-layout>
    <div class="py-8 max-w-4xl mx-auto">
        <div class="flex justify-between items-center">
            <a href="{{ route('idea.index') }}" class="flex items-center gap-x-2 text-sm"><i
        ...
        <div class="mt-8 space-y-6">
            @if ($idea->image_path)
                <div class="rounded-lg overflow-hidden">
                    <img src="{{ asset(path: 'storage/' . $idea->image_path) }}" class="w-full h-auto object-cover" />
                </div>
            @endif
            ...
            @if ($idea->description)
                <x-card class="mt-6" is="div">
                    <div class="text-foreground max-w-none cursor-pointer prose prose-invert">{!! $idea->formattedDescription !!}
                    </div>
                </x-card>
            @endif
                                ...
                                <form action="{{ route('step.update', $step) }}" method="POST">
                                    ...
                                    <div class="flex items-center gap-x-3">
                                        <button type="submit" role="checkbox"
                                            class="size-5 flex items-center justify-center rounded-lg text-primary-foreground {{ $step->completed ? 'bg-primary' : 'border border-primary' }}">&check;</button><span
                ...
                <div>
                    ...
                    <div class="mt-3 space-y-2">
                        @foreach ($idea->links as $link)
                            <x-card :href="$link" class="text-primary font-medium flex gap-x-3 items-center"><i
                                    class="fa-solid fa-up-right-from-square"></i>{{ $link }}</x-card>
                        @endforeach
                    </div>
                </div>
            @endif
